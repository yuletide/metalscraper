...
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.119 Safari/537.36'}
genres = ('heavy', 'black', 'death', 'doom', 'thrash', 'speed', 'folk',
          'power', 'prog', 'electronic', 'gothic', 'orchestral', 'avantgarde')
...
def scrape_bands(limit=''):
    ...
    bands = scraperwiki.sqlite.select(
        "* FROM swdata where scraped IS NULL" + limit)
    for band in bands:
        scrape_band(band)
        sleep(random()*2)
...
def scrape_band(band):
    ...
    if band['link']:
        ...
        keys = list(
            map(lambda x: x.text[:-1], driver.find_elements_by_tag_name('dt')))
        vals = list(
            map(lambda x: x.text, driver.find_elements_by_tag_name('dd')))
        for i, key in enumerate(keys):
            if key == 'Location':
                band['location'] = vals[i]
                # .encode('ISO-8859-1').decode('utf-8')
                band['location_utf'] = vals[i]
                ...
                band['current_label'] = vals[i]
        band['comment'] = driver.find_element_by_class_name(
            'band_comment').text
        save_band(band)
        geocode_band(band)
        return True
...
def geocode_band(band):
    ...
    if band['location_utf'] and band['country']:
        ...
        if '(early)' in location:
            ...
            # take the original location or the later one?
            location = location.split('(early)')[0]
